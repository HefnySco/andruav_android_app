...
abstract class CameraCapturer implements CameraVideoCapturer {
  ...
  private final CameraSession.Events cameraSessionEventsHandler = new CameraSession.Events() {
    ...
    @Override
    public void onCameraZoomChanged(CameraSession session, final float zoom) {
      /*checkIsOnCameraThread();
      synchronized (stateLock) {
        if (session != currentSession) {
          Logging.w(TAG, "onCameraError from another session.");
          return;
        }
      }*/
      eventsHandler.onCameraZoomChanged(zoom);
    }

    @Override
    public void onCameraFlashChanged(CameraSession session, final int flashOn) {
      /*checkIsOnCameraThread();
      synchronized (stateLock) {
        if (session != currentSession) {
          Logging.w(TAG, "onCameraError from another session.");
      ...
      eventsHandler.onCameraFlashChanged(flashOn);
  ...
  public CameraCapturer(String cameraName, @Nullable CameraEventsHandler eventsHandler,
    ...
    if (eventsHandler == null) {
      eventsHandler = new CameraEventsHandler() {
        ...
        @Override
        public void onCameraFlashChanged(final int onOff){}
        @Override
