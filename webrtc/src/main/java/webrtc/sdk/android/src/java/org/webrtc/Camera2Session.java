...
class Camera2Session implements CameraSession {
  ...
  private int isTorchOn = AndruavWebRTCGlobals.FlashOff;
  ...
  private void start() {
    ...
    {
      isTorchOn = AndruavWebRTCGlobals.FlashDisabled;
    }

    events.onCameraFlashChanged(this, isTorchOn);
  }
  ...
  public void setZoom (float zoom)
  {
    ...
    try
    {
      ...
      setZoomPrivate (captureRequestBuilder, zoom);

      captureRequestBuilder.addTarget(surface);
      captureSession.setRepeatingRequest(captureRequestBuilder.build(), new CameraCaptureCallback(), cameraThreadHandler);

    } catch (CameraAccessException e) {
      reportError("Failed to setZoom.");
      return ;
    }

    actualZoom = zoom;

    events.onCameraZoomChanged (this, zoom);

  }

  private void setZoomPrivate (final CaptureRequest.Builder captureRequestBuilder, final float zoom)
  {

    if( !isZoomSupported())
    {
      reportError("Zoom not supported.");
      return ;
    }

    final Rect sensor_rect = cameraCharacteristics.get(CameraCharacteristics.SENSOR_INFO_ACTIVE_ARRAY_SIZE);
    int left = sensor_rect.width()/2;
    int right = left;
    int top = sensor_rect.height()/2;
    int bottom = top;
    final float actualZoom = zoom * zoomMax;
    final int hwidth = (int)(sensor_rect.width() / (2.0*actualZoom));
    final int hheight = (int)(sensor_rect.height() / (2.0*actualZoom));
    ...
    final Rect scalar_crop_region = new Rect(left, top, right, bottom);

    if( scalar_crop_region == null ) return ;

    captureRequestBuilder.set(CaptureRequest.SCALER_CROP_REGION, scalar_crop_region);
  }
  ...
  @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
  @Override
  public void setFlash (final int flashOn)
  {
    if (isCameraFrontFacing)
    {
      events.onCameraFlashChanged (this, AndruavWebRTCGlobals.FlashDisabled );
      return ;
    }

    if ((flashOn != AndruavWebRTCGlobals.FlashOn) && (flashOn != AndruavWebRTCGlobals.FlashOff))
    {
      // bad values.
      return;
    }
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
      ...
      try {
        ...
        captureRequestBuilder.set(CaptureRequest.FLASH_MODE, flashOn==AndruavWebRTCGlobals.FlashOn?CameraMetadata.FLASH_MODE_TORCH:CameraMetadata.FLASH_MODE_OFF);
        ...
        events.onCameraFlashChanged (this, flashOn);
  ...
  public int getFlash ()
  {
    if (isCameraFrontFacing) return AndruavWebRTCGlobals.FlashDisabled;
