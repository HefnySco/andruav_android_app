...
public class AndruavFacade extends AndruavFacadeBase{
    ...
    /***
     * Send a request to subscribe/unsubscribe in IMU stream in a target
     * @param startIMU
     * @param target
     */
    public static void ControlIMU (final boolean startIMU, final AndruavUnitBase target)
    {
        if ((target == null) && startIMU) return; // cannot ask all in same time

        final AndruavMessage_RemoteExecute andruavMessage_remoteExecute = new AndruavMessage_RemoteExecute();
        andruavMessage_remoteExecute.RemoteCommandID = AndruavMessage_RemoteExecute.RemoteCommand_IMUCTRL;
        andruavMessage_remoteExecute.Variables.put("Act",String.valueOf(startIMU));
    ...
    /**
     *
     * @param numberofImages number of images to take.
     *                       <br> Implemented but no GUI for it.
     * @param timeBetweenShots time between shots in milliseconds.
     *                         <br> Implemented but no GUI for it.
     * @param SendBackImages sendMessageToModule a small image to sender or just take image and store it locally in target unit.
     *                       <br><b>TRUE and NOT used</b>
     * @param target target unit name
     */
    public static void takeImage (final int numberofImages , final long timeBetweenShots, final boolean SendBackImages, final AndruavUnitBase target)
    {
        ...
        final AndruavMessage_Ctrl_Camera andruavResala_ctrl_camera = new AndruavMessage_Ctrl_Camera();
        andruavResala_ctrl_camera.CameraSource = AndruavMessage_Ctrl_Camera.CAMERA_SOURCE_MOBILE;
        andruavResala_ctrl_camera.NumberOfImages = numberofImages;
        andruavResala_ctrl_camera.TimeBetweenShotes = timeBetweenShots;
        andruavResala_ctrl_camera.DistanceBetweenShotes = 0;
        andruavResala_ctrl_camera.SendBackImages = SendBackImages;

        sendMessage(andruavResala_ctrl_camera,target, Boolean.FALSE);

    }


    public static void switchCamera(final AndruavUnitBase target, final String CameraUID)
    {
        if (!AndruavSettings.andruavWe7daBase.canImage())
        {
            return;
        ...
        final AndruavMessage_CameraSwitch andruavMessage_cameraSwitch = new AndruavMessage_CameraSwitch();
        andruavMessage_cameraSwitch.CameraUniqueName = CameraUID;

        sendMessage(andruavMessage_cameraSwitch,target, Boolean.FALSE);
    }

    /***
     * Start or stop video recording.
     * @param startRecodring true to start recording, and false to stop.
     * @param target target unit name.
     */
    public static void recordVideo(final boolean startRecodring, final AndruavUnitBase target)
    {
        final AndruavMessage_RemoteExecute andruavMessage_remoteExecute = new AndruavMessage_RemoteExecute();
        andruavMessage_remoteExecute.RemoteCommandID = AndruavMessage_RemoteExecute.RemoteCommand_RECORDVIDEO;
        andruavMessage_remoteExecute.Variables.put("Act",String.valueOf(startRecodring));

        sendMessage(andruavMessage_remoteExecute, target, Boolean.FALSE);
    }


    public static void streamVideoToggle(final AndruavUnitBase andruavUnitBase) {

        if (andruavUnitBase.getIsCGS())
        {
            // cannot stream video from GCS
            andruavUnitBase.VideoStreamingActivated = false;
            return ;
        }
        andruavUnitBase.VideoStreamingActivated = ! andruavUnitBase.VideoStreamingActivated;
        streamVideo(andruavUnitBase.VideoStreamingActivated, andruavUnitBase);
    }


    public static void getCameraList (final AndruavUnitBase andruavUnitBase) {
        ...
        final AndruavMessage_RemoteExecute andruavMessage_remoteExecute = new AndruavMessage_RemoteExecute();
        andruavMessage_remoteExecute.RemoteCommandID = AndruavMessage_CameraList.TYPE_AndruavCMD_CameraList;
    ...
    public static void streamOffAllVideo ()
    {
        ...
        for (int i=0;i<s;++i)
        {
            final AndruavUnitBase andruavUnitBase =  andruavUnitMapBase.valueAt(i);
            if (andruavUnitBase.VideoStreamingActivated)
            {
                andruavUnitBase.VideoStreamingActivated = false;
                streamVideo(false, andruavUnitBase);
            }
        }
    }

    /***
    * sendMessageToModule Stream Video "Request" to a given target to tell it to start/stop video streaming to Me.
    * @param startVideo start/stop video streaming.
    * @param target target name
    */
    public static void streamVideo(final boolean startVideo, final AndruavUnitBase target)
    {
        ...
        final AndruavMessage_RemoteExecute andruavMessage_remoteExecute = new AndruavMessage_RemoteExecute();
        andruavMessage_remoteExecute.RemoteCommandID = AndruavMessage_RemoteExecute.RemoteCommand_STREAMVIDEO;
        andruavMessage_remoteExecute.Variables.put("Act", String.valueOf(startVideo));
        andruavMessage_remoteExecute.Variables.put("CH", target.VideoStreamingChannel);
    ...
    /***
     * sendMessageToModule Stream Video "Request" to a given target to tell it to continue video streaming to Me.
     * @param andruavUnit target name
     */
    public static void streamVideoResume(final AndruavUnitBase andruavUnit)
    {
        if (!AndruavSettings.andruavWe7daBase.canVideo())
        {
            return;
    ...
    public static void MakeTilt(final AndruavUnitBase target)
    {
        final AndruavMessage_RemoteExecute andruavMessage_remoteExecute = new AndruavMessage_RemoteExecute();
        andruavMessage_remoteExecute.RemoteCommandID = AndruavMessage_RemoteExecute.RemoteCommand_MAKETILT;
    ...
    /**
     * Used to order remote target to rotate its video screen.
     * Used by CGS older than HoneyComb , as HoneyComd can rotate image directly locally using View.setRotation()
     * @param target
     */
    public static void RotateCAM (final AndruavUnitBase target)
    {
        if (!AndruavSettings.andruavWe7daBase.canImage())
        {
            return;
        ...
        final AndruavMessage_RemoteExecute andruavMessage_remoteExecute = new AndruavMessage_RemoteExecute();
        andruavMessage_remoteExecute.RemoteCommandID = AndruavMessage_RemoteExecute.RemoteCommand_ROTATECAM;
