...
import rcmobile.andruavmiddlelibrary.factory.math.Angles;
...
public class AttitudeIndicatorView extends View {
    ...
    private void initialize() {
        ...
        planeCenterPaint.setColor(Color.parseColor("#F75050")); // btn_TXT_ERROR
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        skyPaint.setShader(new LinearGradient(0, -radiusInternal, 0, radiusInternal, Color
                .parseColor("#75A4D3"), Color.parseColor("#3671AB"), Shader.TileMode.CLAMP));

        groundPaint.setShader(new LinearGradient(0, radiusInternal, 0, radiusInternal, Color
                .parseColor("#75D375"), Color.parseColor("#36AB36"), Shader.TileMode.CLAMP));
    ...
    public void setAttitude(final float roll, final float pitch, final float yaw, final boolean useFCB, final boolean inTrackingMode) {
        this.roll = roll * Angles.RADIANS_TO_DEGREES;
        this.pitch = pitch * Angles.RADIANS_TO_DEGREES;
        this.yaw = yaw * Angles.RADIANS_TO_DEGREES;

        if ((museFCB != useFCB) || (minTrackingMode != inTrackingMode)) {
            museFCB = useFCB;
            minTrackingMode = inTrackingMode;

            if (useFCB) {
                if (inTrackingMode)
                {
                    planePaint.setColor(Color.RED);
                    planeCenterPaint.setColor(Color.parseColor("#C96771")); // btn_TXT_ERROR
                }
                else {
                    planePaint.setColor(Color.YELLOW);
                    planeCenterPaint.setColor(Color.parseColor("#D3D375")); // btn_TXT_ERROR
                }
